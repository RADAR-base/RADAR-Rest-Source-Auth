...
class GarminServiceUserRepository(
    ...
    private val userListReader: ObjectReader by lazy { objectMapper.readerFor(Users::class.java) }
    private val userReader: ObjectReader by lazy { objectMapper.readerFor(GarminUser::class.java) }
    private val oauthReader: ObjectReader by lazy { objectMapper.readerFor(OAuth1UserCredentials::class.java) }
    private val signedRequestReader: ObjectReader by lazy { objectMapper.readerFor(SignRequestParams::class.java) }
    ...
    @Throws(IOException::class)
    private fun requestFor(relativeUrl: String): Request.Builder = Request.Builder().apply {
        url(baseUrl.resolve(relativeUrl)
            ?: throw IllegalArgumentException("Relative URL is invalid"))
        header("Authorization", requestAuthorization())
    }
